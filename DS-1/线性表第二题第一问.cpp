...
1）设计一个算法求两个集合 A 和 B 的差运算， 即 C=A-B， 要求算法的空间复杂度为 O(1), 并释放单链表 A 和 B 中不需要的节点。
时间复杂度为O(n^2) 
*********/
...
void Subtract(LinkNode*& L1,LinkNode*& L2);//求L1,L2差运算 
...
int main()
{
	...
	int a1[n1-1];
	int a2[n2-1];
	cout<<"请输入链表L1的"<<n1<<"个互不相同的元素：";
	for(int i=0;i<n1;i++)
	{
		cin>>a1[i];
	}
	cout<<"请输入链表L2的"<<n2<<"个互不相同的元素：";
	for(int i=0;i<n2;i++)
	{			
		cin>>a2[i];
	}
